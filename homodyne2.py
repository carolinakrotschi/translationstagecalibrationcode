...
class HomodyneGui:
    ...
    def build_ui(self):
        ...
        if plt is None or FigureCanvasTkAgg is None:
            ...
            self.plot_figure = plt.Figure(figsize=(8, 6), dpi=100)
            self.plot_axes = self.plot_figure.subplots(3, 1)
            if isinstance(self.plot_axes, np.ndarray):
                self.plot_axes = self.plot_axes.tolist()
            elif not isinstance(self.plot_axes, (list, tuple)):
                self.plot_axes = [self.plot_axes]

            self.plot_axes[0].set_title("S1 raw")
            self.plot_axes[1].set_title("S2 raw")
            self.plot_axes[2].set_title("Ref raw")

            for ax in self.plot_axes:
                ax.grid(True, linestyle=':', alpha=0.6)

            self.plot_axes[0].set_ylabel("Voltage")
            self.plot_axes[1].set_ylabel("Voltage")
            self.plot_axes[2].set_ylabel("Voltage")
            self.plot_axes[2].set_xlabel("Samples")

            self.plot_lines = {
                'S1_raw': self.plot_axes[0].plot([], [], color='blue')[0],
                'S2_raw': self.plot_axes[1].plot([], [], color='green')[0],
                'Ref_raw': self.plot_axes[2].plot([], [], color='red')[0]
            }
