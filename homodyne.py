...
class HomodyneGui:
    ...
    def poll_stage_status(self):
        if self.stage_connected:
            ...
            if self.stage_target_position is not None and not self.calibrating:
                moved = abs(pos - self.stage_start_position)
    ...
    def calibration_stage_motion(self):
        ...
        self.root.after(
            ...
            self.status.configure(
                text=(
                    f"Calibration sweep: "
                    f"+/-{CALIBRATION_STAGE_DISTANCE_MM:.5f} mm"
                ),
                text_color=ORANGE_COLOR
            )
        )
    ...
    def maybe_start_lock_correction(self, drift_mm, correction_mm):
        ...
        self.set_stage_position_label_text(
            f"Stage target: {target_position_mm:.6f} mm"
        )
    ...
    def finish_lock_correction(self, position_mm):
        ...
        if position_mm is not None:
            ...
            self.set_stage_position_label_text(
                f"Stage position: {position_mm:.6f} mm"
            )
