...
# So that you do not have to understand every line of the code, I will now explain the complete path of a signal through all these classes (following the structure of 0.)
    ...
    # 6. Signals are normalized: HomodyneQuadratureCounter : normalize() : converts the raw voltages into centered and scaled signal coordinates
    # 7. Phase is computed: HomodyneQuadratureCounter : update() : uses atan2(s2, s1) to calculate the instantaneous phase
    # 8. Phase jumps are removed: HomodyneQuadratureCounter : update() : uses wrap_to_pi() and unwrapped_phase_rad to build a continuous phase trace
    # 9. Fringe position is computed: HomodyneQuadratureCounter : update() : converts unwrapped_phase_rad into fringe_position by dividing by 2*pi
    # 10. Whole fringes are counted: HomodyneQuadratureCounter : update() : turns fringe_position into signed_fringes and fringe_delta
    # 11. Direction is estimated: HomodyneQuadratureCounter : update() : uses the recent phase history and a hysteresis rule to decide forward, backward, or none
    # 12. Final sample object is created: HomodyneQuadratureCounter : update() : returns a HomodyneSample containing all computed values
    # 13. Sample is stored for the GUI: HomodyneGui : measurement_loop() : writes latest_sample and latest_distance_mm
    # 14. Distance is converted to millimeters: HomodyneQuadratureCounter : signed_distance_mm() : multiplies the fringe count by fringe_distance_mm
    # 15. GUI updates the labels: HomodyneGui : update_ui_loop() : reads latest_sample and updates the text fields
    # 16. GUI updates the plot: HomodyneGui : update_plot() and update_plot_data() : redraws raw signals, fit lines, and the Lissajous circle
