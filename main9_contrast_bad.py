...
# jetzt mit contrast statt intensity
...
class InterferometerApp(ctk.CTk):
    def __init__(self):
        ...
        # Zeitliche Fringe-Zählung über Contrast
    ...
    def loop(self):
        ...
        while self.is_monitoring:
            ...
            if img is not None:
                ...
                contrast = self.camera_handler.get_contrast_from_frame(img)
                visible_fringes = self.camera_handler.count_visible_fringes_from_frame(img)

                if contrast is not None:
                    self.after(
                        0,
                        lambda v=contrast: self.label_intensity.configure(
                            text=f"Contrast: {v:.4f}"
                        )
                    )

                    self.update_accumulated_fringes(contrast)
